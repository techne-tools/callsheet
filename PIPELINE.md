...
- **Release (Hermes, 2026-08-18):** v0.1.0 shipped. Repo made public (`prismatic7/callsheet`), PII audit done (personal paths scrubbed 8e2e774, no secrets/hostnames in tree or history). CI (ci.yml: npm ci → type-check → vite build → vitest → cargo test) green. Release workflow (release.yml: tauri-action matrix, macOS arm64/x64 + Linux + Windows) green — 10 assets attached. ⚠️ First tag run failed: `Resource not accessible by integration` (HTTP 403) on createRelease — known new-repo quirk despite `contents: write`. Fix: manual `gh release create v0.1.0 --draft`, rerun failed jobs, publish. Future tags work without the manual step. Changelog: CHANGELOG.md. Project register updated (Completed, v6).
- **OpenDesign review + fix pass (Hermes, 2026-08-18):** OpenDesign review (`DESIGN_REVIEW-COMPLETE.md`, committed) — verdict strong/on-voice, no P0s; 9-item action list. All 9 closed in one commit cff954b: (1) window presence modes wired via day-header menu, (2) custom activity-type UI in sidebar (create/delete; backend guards seed + in-use types with clear errors + regression test), (3) ghost-card propose affordance (◌) — manual placeholder until the Hermes/Noema agent layer is wired, (4) dead `move_card` removed (CONTRACT.md updated) + `.day-pane--over` now applied during drag-over + `deriveTextSecondary` removed, (5) per-card type picker (colour-dot menu), (6) Today button + Cmd+T, (7) non-colour type label on cards (a11y), (8) quiet red delete hover, (9) SVG delete icon + collapsed CSS. Gates: build PASS, vitest 45/45, cargo 7/7.
